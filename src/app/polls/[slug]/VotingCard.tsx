...
export default function VotingCard({ 
  ...
  return (
    <div className="bg-slate-900 border border-slate-800/80 rounded-4xl flex flex-col transition-all duration-500 hover:-translate-y-2 hover:border-orange-500/40 hover:shadow-[0_20px_40px_rgb(249,115,22,0.15)] relative overflow-hidden group w-full max-w-85 min-w-0">
      ...
      {/* AVATAR */}
      <div className="relative pt-10 px-6 flex flex-col items-center text-center z-10 transition-transform duration-500">
        {hasValidImage ? (
        ...
        {status === "success" ? (
          ...
          <div className="animate-fade-in-up space-y-4 min-w-0">
            ...
            <div className="flex gap-2 pt-2">
              <button onClick={handleXShare} className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-950 hover:bg-black text-white text-[10px] font-bold uppercase tracking-widest rounded-xl border border-slate-800 hover:border-slate-700 transition-all">
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 22.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                Share
              </button>
              <button onClick={handleWhatsAppShare} className="flex-1 flex items-center justify-center gap-2 py-3 bg-[#25D366]/10 hover:bg-[#25D366] text-[#25D366] hover:text-white text-[10px] font-bold uppercase tracking-widest rounded-xl border border-[#25D366]/20 transition-all">
          ...
          /* FORM STATE */
          <form onSubmit={handleVote} className="animate-fade-in-up space-y-3 min-w-0 w-full mt-auto">
            <div className="flex items-center justify-between mb-1 border-b border-slate-800/80 pb-2">
              <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" /> Secure Vote
              </span>
              <button 
                type="button" 
            ...
            {status === "error" && message && (
              <div className="flex items-start gap-2 p-3 bg-red-500/10 border border-red-500/20 rounded-xl text-red-400 text-xs font-medium w-full">
            ...
            <input
              ...
              className="w-full bg-slate-950/80 border border-slate-800 text-slate-100 rounded-xl px-4 py-3.5 text-sm focus:outline-none focus:border-orange-500/70 focus:ring-1 focus:ring-orange-500/70 transition-all placeholder:text-slate-600 disabled:opacity-50 shadow-inner"
            />

            <div className="flex justify-center bg-slate-950/60 rounded-xl overflow-hidden border border-slate-800/80 w-full min-h-16.25 relative backdrop-blur-md">
              {!turnstileToken && status !== "error" && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <Loader2 className="w-4 h-4 text-orange-500 animate-spin mr-2" />
                  <span className="text-[9px] text-slate-400 font-mono uppercase tracking-widest">Securing connection...</span>
                </div>
              )}
              <div className="relative z-10 w-full flex justify-center py-1.5">
                <Turnstile
            ...
            <button
              ...
              className="w-full py-3.5 bg-orange-500 hover:bg-orange-600 text-slate-950 font-black uppercase tracking-widest rounded-xl transition-all shadow-[0_0_20px_rgba(249,115,22,0.2)] hover:shadow-[0_0_30px_rgba(249,115,22,0.4)] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 group/btn"
            >
              {status === "loading" ? (
                <>
          ...
          <div className="mt-auto animate-fade-in-up">
            <button
              ...
              className="w-full py-4 bg-slate-800/50 hover:bg-white text-slate-300 hover:text-slate-950 font-bold uppercase tracking-widest text-sm rounded-2xl backdrop-blur-md border border-slate-700 hover:border-white transition-all duration-300 hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 group/vote shadow-[0_0_20px_rgba(0,0,0,0.2)] hover:shadow-[0_0_30px_rgba(255,255,255,0.2)]"
            >
              <span>Vote for {candidateName.split(' ')[0]}</span>
              <ArrowRight className="w-4 h-4 group-hover/vote:translate-x-1 transition-transform text-orange-500 group-hover/vote:text-slate-950" />
            </button>
          </div>
        )}
